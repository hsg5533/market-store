...
module.exports = {
  payments: function (e, o) {
    ...
    axios({
      ...
      console.log("인증 토큰: " + e.data.response.access_token),
        axios({
          url: `https://api.iamport.kr/payments/${r}`,
          method: "get",
          headers: { Authorization: e.data.response.access_token },
        }).then((e) => {
          console.log("조회한 결제 정보: "), console.log(e.data.response);
          var r =
          ...
          console.log("결제 금액 계산: " + r),
            conn.query(r, [t, t], function (r, t) {
              if (r) console.log("결제 금액 계산 오류: " + r);
              else if (e.data.response.amount == parseInt(t[0].total)) {
                var a =
                  "insert into pay (orderId,paymentKey,pg_provider,emb_pg_provider,price) values (?,?,?,?,?)";
                console.log("결제 기록 등록: " + a);
                var s = [
                  e.data.response.merchant_uid,
                  e.data.response.imp_uid,
                  e.data.response.pg_provider,
                  e.data.response.emb_pg_provider,
                  e.data.response.amount,
                ];
                conn.query(a, s, function (r) {
                  if (r) console.log("결제기록 등록 오류: " + r);
                  else {
                    var t = "update `order` set pay='y' where orderid=?";
                    console.log("주문서 결제 기록 수정: " + t);
                    var a = [e.data.response.merchant_uid];
                    conn.query(t, a, function (r) {
                      if (r) console.log("주문서 결제기록 수정 오류: " + r);
                        ...
                        conn.query(t, a, function (r) {
                          if (r)
                            console.log("주문상품 결제기록 수정 오류: " + r);
                          else
                            switch (
                              (console.log(
                                "결제 상태: " + e.data.response.status
                              ),
              ...
              } else o.send({ status: "forgery", message: "위조된 결제시도" });
            });
        });
    });
  },
